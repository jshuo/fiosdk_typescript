...
const BIP32Path_1 = require("./utils/BIP32Path");
const secux_1 = require("./utils/secux");
/**
 * @ignore
 */
...
class FIOSDK {
     ...
     */
    constructor(privateKey, publicKey, baseUrl, fetchjson, hwtransport, registerMockUrl = '', technologyProviderId = '', returnPreparedTrx = false) {
        /**
         * Defines whether SignedTransaction would execute or return prepared transaction
         */
        this.returnPreparedTrx = false;
        this.transactions = new Transactions_1.Transactions(hwtransport);
    ...
    static getPublicKey(transport, path) {
        return __awaiter(this, void 0, void 0, function* () {
            const { pathBuffer } = (0, BIP32Path_1.buildPathBuffer)(path);
            const [cla, ins] = [0x80, 0xc1];
            const rsp = yield transport.Send(cla, ins, 0, 0, pathBuffer);
            const publicKey = rsp.data;
            return (0, secux_1.toCompressed)(publicKey);
        });
    }
    /**
     * hash a pub key
     ...
     * @param offset First request from list to return. If omitted, 0 is assumed.
     */
    getReceivedFioRequests(limit, offset) {
